...
void Hp_6_init(void)
{
	...
	GPIOC->OSPEEDR &= ~(3 << 26);
	GPIOC->OSPEEDR |= 2 << 26;  	//配置PC13 的输出速度 50MHZ
...
**-----------------------------------------------------------------------
*****************************************************************/
void Hp_6_I2CWrite(uint8_t Hp_6_address, uint8_t *pData, uint8_t bytes)
{
	uint8_t i;
	HP6IIC_Start();
...
bool Hp_6_I2CRead(uint8_t Hp_6_address, uint8_t *pData, uint8_t bytes)
{
  ...
  /* 从机就会给出数据 */
  *(pData+23) = HP6IIC_ReadByte(1);//注意：读取byte[23]数据无需应答    
  HP6IIC_Stop();
	return 0;
}
